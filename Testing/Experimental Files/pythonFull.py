...
def movieMaker(fig, ims, title, save_dir):
    ...
    im_ani = animation.ArtistAnimation(
        fig, ims, interval=50, repeat_delay=3000, blit=True)
    print('Check')
...
if __name__ == '__main__':
    ...
    if loadData:
        adc_data = np.fromfile(
            'Testing/RadarBins/boxing_6s.bin', dtype=np.uint16)
    ...
    for i, frame in enumerate(dataCube):
        ...
        det_matrix, aoa_input = dsp.doppler_processing(
            radar_cube, num_tx_antennas=1, clutter_removal_enabled=True, window_type_2d=Window.HAMMING)
        ...
        x, y, z = dsp.naive_xyz(azimuthInput.T, 1, 4)
        ...
        if plot2DscatterXY or plot2DscatterXZ:
            ...
            ims.append(
                (nice.scatter(xyzVec[0], xyzVec[1], xyzVec[2], c='r', marker='o', s=2),))
